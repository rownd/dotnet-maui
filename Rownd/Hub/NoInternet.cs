...
namespace Rownd.Maui.Hub
{
    ...
    {
        public static string Build(RowndInstance rownd)
        {
            var appConfig = rownd.Store.State.AppConfig;
            var fontSize = rownd.Config.Customizations.DefaultFontSize;
            var primaryColor = appConfig?.Config?.Customizations?.PrimaryColor ?? "#5b13df";
        ...
        <html>
            <head>
                ...
                <style>
                    {{NoInternetCSS(fontSize, backgroundColor.ToHex(), primaryColor, isDarkMode)}}
                </style>
                <script>
                    function tryAgain() {
    ...
    """.Trim();
        ...
        private static string NoInternetCSS(double fontSize, string backgroundColor, string primaryColor, bool isDarkMode)
        {
            return $$"""
    body {
